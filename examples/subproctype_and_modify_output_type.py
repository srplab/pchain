...
def OnOutputDataToEnv(selfobj,PCCell,PCProc,DataList) :
  if PCProc.GetType().Equals(CreateTestFileProc) :
    for index in range(DataList._Number):
      item = DataList[index]
      if item.GetType().Equals(FileClass) :
        rawitem = pydata.UnWrap(item)
        if rawitem.value().name.startswith('capitalsquiz_answers') :
          DataList[index] = AnswerKeyFileClass(rawitem.value())
        else :
          DataList[index] = QuizFileClass(rawitem.value())
...
result = realm.RunProc((TotalClass(3),g_capitals),None,CreateIndexProc,CreateTestFileProc,WriteContentFileProc,CloseContentFileProc)
